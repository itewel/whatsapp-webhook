 ...
 * Secure, production-ready webhook handler
 */
...
function verifySignature(req, res, buf) {
  ...
  if (!signature) return; // will be caught later
...
function handleMessage(msg, metadata, contacts) {
  ...
  log("INFO", `📩 New message from ${name}`, { from, type, msgId: msg.id });

  switch (type) {
    case "text":
      log("INFO", "Text message", { text: msg.text.body });
      sendReply(from, `مرحباً ${name}! 👋\nاستلمنا رسالتك: "${msg.text.body}"`);
      break;

    case "image":
      log("INFO", "Image received", { mediaId: msg.image.id });
      sendReply(from, `استلمنا صورتك بنجاح! 🖼️`);
      break;

    case "audio":
      log("INFO", "Audio received", { mediaId: msg.audio.id });
      sendReply(from, `استلمنا رسالتك الصوتية! 🎙️`);
      break;

    case "document":
      log("INFO", "Document received", { filename: msg.document.filename });
      sendReply(from, `استلمنا المستند: ${msg.document.filename} 📄`);
      break;

    case "location":
      log("INFO", "Location received", { lat: msg.location.latitude, lng: msg.location.longitude });
      sendReply(from, `استلمنا موقعك! 📍`);
      break;
...
function handleStatus(status) {
  ...
  log("INFO", `${icons[status.status] || "?"} Message ${status.status}`, {
    msgId: status.id,
...
async function sendReply(to, text) {
  try {
    ...
    log("INFO", "Reply sent", { to, msgId: data.messages?.[0]?.id });
  } catch (err) {
